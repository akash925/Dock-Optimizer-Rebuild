...
export default function TopNav() {
  ...
  // Initialize real-time WebSocket connection for instant notification updates
  ...
  // Enhanced notifications with real-time WebSocket updates
  const { data: notifications = [] } = useQuery<EnhancedNotification[]>({
    queryKey: ["/api/notifications/enhanced"],
    queryFn: async () => {
      console.log('[TopNav] Fetching notifications from /api/notifications/enhanced');
      const response = await fetch('/api/notifications/enhanced');
      if (!response.ok) throw new Error('Failed to fetch notifications');
      const data = await response.json();
      console.log('[TopNav] Received notifications:', data.length, 'items');
      return data;
    },
    enabled: !!user,
    refetchOnWindowFocus: true,
    // Remove manual polling since WebSocket provides real-time updates
  });
  
  // Debug real-time connection status
  useEffect(() => {
    if (wsConnected) {
      console.log('[TopNav] ✅ WebSocket connected - real-time notifications enabled');
    } else {
      console.warn('[TopNav] ❌ WebSocket disconnected - notifications will not update in real-time', socketError);
    }
  }, [wsConnected, socketError]);
    ...
    <header className="bg-white shadow-sm h-16 flex items-center justify-between px-4 sticky top-0 z-40">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-2">
          {/* WebSocket Status Indicator */}
          {wsConnected ? (
            <div className="w-2 h-2 bg-green-500 rounded-full" title="Real-time updates connected" />
          ) : (
            <div className="w-2 h-2 bg-red-500 rounded-full animate-pulse" title={`Real-time updates disconnected${socketError ? `: ${socketError}` : ''}`} />
          )}
